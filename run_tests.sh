...
reiniciar_funciones_y_pruebas() {
    ...
    osascript -e 'tell application "Terminal" to do script "cd /Volumes/SSD_JOEL/TT/TT-1/PSOYGA && source env_tests/bin/activate && pytest -s tests/test_optimizado.py"'
...
echo "Esperando cambios en el archivo 'test_optimizado.py'..."

# Monitorear el archivo `test_optimizado.py` para detectar cambios
inotifywait -e close_write tests/test_optimizado.py

# Cuando se detecte un cambio (guardar el archivo), ejecutar el proceso
echo "Cambio detectado en 'test_optimizado.py'. Reiniciando el proceso..."
reiniciar_funciones_y_pruebas
