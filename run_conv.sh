...
reiniciar_funciones_y_pruebas() {
    ...
    osascript -e 'tell application "Terminal" to do script "cd /Volumes/SSD_JOEL/TT/TT-1/PSOYGA && source env_tests/bin/activate && pytest -s tests/datos_conv.py"'
...
echo "Esperando cambios en el archivo 'datos_conv.py'..."

# Monitorear el archivo `datos_conv.py` para detectar cambios
inotifywait -e close_write tests/datos_conv.py

# Cuando se detecte un cambio (guardar el archivo), ejecutar el proceso
echo "Cambio detectado en 'datos_conv.py'. Reiniciando el proceso..."
reiniciar_funciones_y_pruebas
